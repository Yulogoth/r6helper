...
void ImGuiConsole::Draw()
{
    ...
    if (!ImGui::Begin(m_ConsoleName.data(), nullptr, ImGuiWindowFlags_MenuBar | ImGuiWindowFlags_NoDecoration | ImGuiWindowFlags_NoMove))
    {
        ImGui::PopStyleVar();
        ImGui::End();
        return;
...
void ImGuiConsole::RegisterConsoleCommands()
{
    ...
    m_ConsoleSystem.RegisterCommand("script", "Run given script", [this](const csys::String &filter)
    {
            // Logs command.
            m_ConsoleSystem.RunScript(filter.m_String);
    }, csys::Arg<csys::String>("script_name"));

    m_ConsoleSystem.RegisterCommand("savecfg", "Saves R6H cfg", [this](const csys::String &name) {
        if (name.m_String == "") {
            //Config::SaveCFG("cfg.r6h");
            m_ConsoleSystem.Log(csys::ItemType::INFO) << "Successfully loaded config 'cfg.r6h'" << csys::endl;
        }
        else {
            //Config::SaveCFG(name.m_String);
            m_ConsoleSystem.Log(csys::INFO) << "Successfully loaded config '" << name.m_String << "'!" << csys::endl;
        }
    }, csys::Arg<csys::String>("config_name"));

    m_ConsoleSystem.RegisterCommand("loadcfg", "Loads config", [this](const csys::String &name) {
        if (name.m_String == "") {
            //Config::LoadCFG("cfg.r6h");
            m_ConsoleSystem.Log(csys::ItemType::INFO) << "Succcessfully saved config 'cfg.r6h'!" << csys::endl;
        }
        else {
            //Config::LoadCFG(name.m_String);
            m_ConsoleSystem.Log(csys::INFO) << "Successfully saved config '" << name.m_String << "'!" << csys::endl;
        }
    }, csys::Arg<csys::String>("config_name"));

    /*m_ConsoleSystem.RegisterCommand("run", "Runs R6 with params 'platform be'\n[PLATFORM]\nUplay = 0\nSteam = 1\nEpic Games = 2\n[BattlEye]\nOn = 0\nOff = 1", [this](const csys::String& platform, const csys::String& be) {
        if (platform.m_String == "0") g_Options.platform == 0;
        if (platform.m_String == "1") g_Options.platform == 1;
        if (platform.m_String == "2") g_Options.platform == 2;
        else g_Options.platform == 0;
        
        if (be.m_String == "0") g_Options.be == false;
        if (be.m_String == "1") g_Options.be == true;
        else g_Options.be == false;

        R6.R6Start();
    }, csys::Arg<csys::String>("platform"), csys::Arg<csys::String>("be"));*/

    m_ConsoleSystem.RegisterCommand("exit", "Exit R6Helper", [this]() {
        quick_exit(0);
    });
}
