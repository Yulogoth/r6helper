...
namespace csys
{
    ...
    CSYS_INLINE void System::RunScript(const std::string &script_name)
    {
        ...
        {
            m_ItemLog.log(ERRORE) << "Script \"" << script_name << "\" not found" << csys::endl;
            return;
        }
    ...
    CSYS_INLINE void System::ParseCommandLine(const String &line)
    {
        ...
        if (command == m_Commands.end())
            Log(ERRORE) << s_ErrorSetGetNotFound << endl;
            // Run the command
        else
        {
            // Get the arguments.
